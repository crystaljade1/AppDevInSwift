...
list.append("Lemon") // can be used to add one value at a time
list.insert("Grape", at: 2) //can be used when there is a specific need for a value to have a specific index value 
list += ["Berry"] // can be used to add more than one value to an array at one time
